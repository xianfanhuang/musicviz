...
class MusicPlayer {
  ...
  cleanupAudioElement() {
    ...
    if (oldSrc && oldSrc.startsWith('blob:')) {
      ...
      if (!stillInUse) {
        try { URL.revokeObjectURL(oldSrc); } catch (e) { console.warn('清理blob URL时出错:', e); }
      }
    }
  }
  ...
  clearPlaylist() {
    ...
    this.playlist.forEach(track => {
      if (track.url && track.url.startsWith('blob:')) {
        try { URL.revokeObjectURL(track.url); } catch (e) { console.warn('清理blob URL失败:', e); }
      }
    });
    this.playlist = [];
    this.currentTrackIndex = 0;
  ...
  startPerformanceMonitor() {
    ...
    if (fpsDisplay && perfLevel && this.visualizer) {
      setInterval(() => {
        ...
        const levelNames = { low: '省电模式', medium: '标准模式', high: '高性能' };
        fpsDisplay.textContent = `FPS: ${fps}`;
        perfLevel.textContent = levelNames[level] || '标准模式';
        if (fps < 30) fpsDisplay.style.color = '#ff6b6b';
        else if (fps < 50) fpsDisplay.style.color = '#ffd93d';
        else fpsDisplay.style.color = '#4facfe';
      }, 1000);
    }
  }
}
